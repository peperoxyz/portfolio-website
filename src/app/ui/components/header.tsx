...
export const Header = () => {
	...
	return (
		<div>
			<div className={`${isSticky ? "sticky top-0" : ""} bg-primary-dark text-white transition-all duration-300 ease-in-out`}>
							...
							{["Home", "Projects", "Articles", "About"].map((item) => (
								<Link
									...
									className={`hover:bg-primary-dark hover:rounded-full hover:px-5 hover:py-2 transition-all px-4 duration-300  ${
										pathname === (item === "Home" ? "/" : `/${item.toLowerCase()}`) ? "text-violet-400 px-5 py-2 rounded-full bg-primary-dark" : "text-white"
									}`}
								>
									{item}
								</Link>
			...
			{/* <div className="h-[30rem] bg-primary-dark"></div> */}
		</div>
	);
};
